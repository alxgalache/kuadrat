...
// Quote request: independent from the inquiry schema above. Same fields plus a
// required Spanish 5-digit postal code (informational; not validated against the
// postal-code DB). The "Más información" textarea maps to `message`.
const quoteRequestSchema = z.object({
  body: z.object({
    ...
    postalCode: z.string().trim().regex(/^[0-9]{5}$/, 'Código postal inválido'),
    message: z.string().trim().min(1, 'La información es obligatoria').max(2000, 'Texto demasiado largo'),
    turnstileToken: z.string().min(1, 'Token de verificación faltante').max(2000, 'Token de verificación inválido'),
  }).strip(),
});

module.exports = { artInquirySchema, quoteRequestSchema };
