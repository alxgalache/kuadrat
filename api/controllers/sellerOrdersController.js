...
/**
 * POST /api/seller/orders/bulk-pickup
 * Schedule a single Sendcloud pickup covering multiple orders.
 */
const scheduleBulkPickup = async (req, res, next) => {
  try {
    const sellerId = req.user.id
    const { orderIds, address, timeSlotStart, timeSlotEnd, specialInstructions } = req.body

    // Validate no duplicate pickups exist for any of these orders
    const placeholders = orderIds.map(() => '?').join(',')
    const existingPickups = await db.execute({
      sql: `SELECT order_id FROM sendcloud_pickups WHERE seller_id = ? AND order_id IN (${placeholders})`,
      args: [sellerId, ...orderIds],
    })
    if (existingPickups.rows.length > 0) {
      const ids = existingPickups.rows.map(r => r.order_id).join(', ')
      throw new ApiError(400, `Ya existe una recogida programada para los pedidos: ${ids}`, 'Recogida duplicada')
    }

    // Load all items for all orders, grouped by order
    const orderItems = [] // { orderId, items: [...], carrierCode, totalWeightKg }
    ...
    for (const orderId of orderIds) {
      ...
      const nonPaid = allItems.filter(i => i.status !== 'paid')
      if (nonPaid.length > 0) {
        throw new ApiError(400, `El pedido #${orderId} tiene artículos que no están en estado "Pagado"`, 'Estado inválido')
      }

      // Get carrier code
      const carrierCode = allItems.find(i => i.sendcloud_carrier_code)?.sendcloud_carrier_code
      if (!carrierCode) {
        throw new ApiError(400, `No se encontró el código del transportista para el pedido #${orderId}`, 'Sin transportista')
      }

      // Verify all orders share the same carrier
      if (!commonCarrier) {
        commonCarrier = carrierCode
      } else if (commonCarrier !== carrierCode) {
        throw new ApiError(400, 'Todos los pedidos seleccionados deben tener el mismo transportista', 'Transportista diferente')
      }
    ...
    // Build Sendcloud pickup request with one item per order
    const pickupResult = await sendcloudProvider.createPickup({
      carrierCode: commonCarrier,
      address: {
        name: address.name,
        companyName: address.companyName || '',
        addressLine1: address.addressLine1,
      ...
      items: orderItems.map(oi => ({
        ...
        totalWeight: oi.totalWeightKg,
      })),
      specialInstructions: specialInstructions || '',
    })

    // Store one pickup row per order, all sharing the same sendcloud_pickup_id
    const pickupAddress = JSON.stringify(address)
    ...
    logger.info({ orderIds, sellerId, pickupId: pickupResult.id, carrierCode: commonCarrier },
      'Bulk pickup scheduled successfully')
 ...
 * Returns same shape as getSellerOrders.
 */
const getSellerShipmentsAdmin = async (req, res, next) => {
  try {
    const { sellerId, status, page = 1, limit = 20 } = req.query

    if (!sellerId) {
      throw new ApiError(400, 'El parámetro sellerId es obligatorio', 'Parámetro faltante')
    }

    const sellerIdNum = parseInt(sellerId, 10)
    if (isNaN(sellerIdNum)) {
      throw new ApiError(400, 'sellerId inválido', 'Parámetro inválido')
    }
    ...
    const [artResult, otherResult] = await Promise.all([
      db.execute({ sql: artSql, args: [sellerIdNum, ...statusArgs] }),
      db.execute({ sql: otherSql, args: [sellerIdNum, ...statusArgs] }),
    ])
...
module.exports = { getSellerOrders, downloadOrderLabel, schedulePickup, scheduleBulkPickup, getSellerShipmentsAdmin }
