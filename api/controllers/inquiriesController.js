...
// Quote request: independent from createArtInquiry above. Same Turnstile + email
// flow, but carries the shipping postal code and uses the quote email template.
const createQuoteRequest = async (req, res, next) => {
  try {
    const { productId, name, email, phone, postalCode, message, turnstileToken } = req.body;

    if (!config.turnstile.secret) {
      logger.error('Turnstile secret not configured; refusing quote request');
      throw new ApiError(503, 'Verificación de seguridad no disponible', 'CAPTCHA_UNAVAILABLE');
    }
    ...
    try {
      await emailService.sendQuoteRequestEmail({
        inquiry: { name, email, phone: phone || null, postalCode, message },
        product: {
          id: Number(product.id),
          name: product.name,
          slug: product.slug,
      ...
      logger.error({ err, productId }, 'Failed to send quote request email');
      throw new ApiError(500, 'No se pudo enviar la solicitud', 'EMAIL_DELIVERY_FAILED');
    }

    return sendSuccess(res, {}, 200, 'Solicitud enviada');
  } catch (err) {
    next(err);
  }
};

module.exports = { createArtInquiry, createQuoteRequest };
