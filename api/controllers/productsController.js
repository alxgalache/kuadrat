...
const getAllProducts = async (req, res, next) => {
  try {
    ...
    let query = `
      ...
      WHERE p.visible = 1 AND p.sold = 0
    `;
    const args = [];
...
const createProduct = async (req, res, next) => {
  try {
    ...
    } else if (!['physical', 'digital'].includes(type)) {
    ...
    const result = await db.execute({
      sql: `
        INSERT INTO products (seller_id, name, description, price, type, basename, slug)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `,
  ...
  try {
    ...
    const productsResult = await db.execute({
      sql: `
        ...
        FROM products p
        LEFT JOIN users u ON p.seller_id = u.id
        WHERE p.seller_id = ? AND p.visible = 1 AND p.sold = 0
