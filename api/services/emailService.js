...
// ---------------------------------------------------------------------------
// Inert transport ("noop")
// ---------------------------------------------------------------------------
// When config.emailTransport is 'noop' NOTHING leaves the process: no Resend
// HTTP call, no SMTP connection. Forced on under NODE_ENV=test so a test run
// can never mail a real recipient of the preproduction environment; available
// elsewhere via EMAIL_TRANSPORT=noop for a safe local environment.
//
// Messages are pushed to an in-memory outbox instead, which doubles as the way
// tests assert on email behaviour (there was no way to do that before).
const isNoopTransport = config.emailTransport === 'noop';

const outbox = [];
let noopCounter = 0;

if (isNoopTransport && !config.isTest) {
  logger.warn('EMAIL_TRANSPORT=noop — email sending is DISABLED, no message will leave this process');
}

// Test-only accessors (the `__` prefix marks them as not part of the public
// email API). Safe to call in any mode; the outbox stays empty when live.
const __getOutbox = () => outbox.slice();
const __clearOutbox = () => { outbox.length = 0; };

// Create transporter. Skipped entirely in noop mode — no SMTP socket is ever
// opened, not even lazily.
const transporter = isNoopTransport ? null : nodemailer.createTransport({
...
const verifyTransporter = async () => {
  if (isNoopTransport) {
    logger.info('Email transport is "noop" - skipping SMTP verification, no emails will be sent');
    return;
  }
  try {
...
// Never created in noop mode.
const resendClient = (!isNoopTransport && config.emailProvider === 'resend')
...
const sendMail = async (options) => {
  // Inert transport: record and return before touching any provider. The shape
  // of the return value matches both providers, so every call site keeps
  // reading `info.messageId` unchanged.
  if (isNoopTransport) {
    const { from, to, subject, html, attachments, replyTo } = options;
    noopCounter += 1;
    const messageId = `noop-${noopCounter}`;
    outbox.push({
      messageId,
      from,
      to,
      ...
      html,
      replyTo,
      attachments: attachments ? attachments.map((a) => a.filename) : [],
      sentAt: new Date().toISOString(),
    });
    logger.debug({ to, subject, messageId }, 'Email suppressed (noop transport)');
    return { messageId };
  }
...
<body bgcolor="#ffffff" style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #ffffff; background: #ffffff;">
    ...
    <tr>
      <td align="center" bgcolor="#ffffff" style="background-color: #ffffff;">
        <table width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 20px 40px 40px; background-color: #ffffff;">
              <h1 style="margin: 0 0 20px; font-size: 24px; font-weight: 600; color: #111827;">¡Bienvenido/a a 140d, ${fullName}!</h1>
              <p style="margin: 0 0 20px; font-size: 16px; line-height: 1.6; color: #374151;">
  ...
  const textContent = `¡Bienvenido/a a 140d, ${fullName}!
...
// Send alert email to admin about items stuck in "arrived" status
const sendStaleArrivedAlertEmail = async (items) => {
  const config = require('../config/env');
  const adminEmail = config.registrationEmail;
  if (!adminEmail) {
    logger.warn('No REGISTRATION_EMAIL configured, skipping stale arrived alert email');
    return;
  }

  const itemRows = items.map(item =>
    `<tr>
      <td style="padding: 8px 12px; border-bottom: 1px solid #e5e7eb; color: #374151;">#${item.order_id}</td>
      <td style="padding: 8px 12px; border-bottom: 1px solid #e5e7eb; color: #374151;">${escapeForEmail(item.product_name)}</td>
      <td style="padding: 8px 12px; border-bottom: 1px solid #e5e7eb; color: #374151;">${item.product_type === 'art' ? 'Arte' : 'Otro'}</td>
      <td style="padding: 8px 12px; border-bottom: 1px solid #e5e7eb; color: #374151; text-align: center; font-weight: 600;">${item.days_stale}</td>
    </tr>`
  ).join('');
...
<head>
  ...
  <meta name="color-scheme" content="light only">
  <meta name="supported-color-schemes" content="light only">
  <title>Alerta: Productos recibidos pendientes de confirmación</title>
  <style>
    :root { color-scheme: light only; }
    @media (prefers-color-scheme: dark) {
      body, table, td, div { background-color: #ffffff !important; color: #111827 !important; }
...
<body bgcolor="#ffffff" style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #ffffff;">
    ...
    <tr>
      <td align="center" bgcolor="#ffffff" style="background-color: #ffffff;">
        <table width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 40px;">
              <h1 style="margin: 0 0 20px; font-size: 24px; font-weight: 600; color: #111827;">Alerta: Productos recibidos pendientes</h1>
              <p style="margin: 0 0 20px; font-size: 16px; line-height: 1.5; color: #374151;">
                Los siguientes <strong>${items.length}</strong> producto(s) llevan más de 10 días en estado <strong>"recibido"</strong> sin ser confirmados por el comprador:
              </p>
              <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse; margin: 0 0 20px;">
                <thead>
                  <tr style="background-color: #f9fafb;">
                    <th style="padding: 8px 12px; text-align: left; font-size: 12px; font-weight: 600; color: #6b7280; border-bottom: 2px solid #e5e7eb;">Pedido</th>
                    <th style="padding: 8px 12px; text-align: left; font-size: 12px; font-weight: 600; color: #6b7280; border-bottom: 2px solid #e5e7eb;">Producto</th>
                    <th style="padding: 8px 12px; text-align: left; font-size: 12px; font-weight: 600; color: #6b7280; border-bottom: 2px solid #e5e7eb;">Tipo</th>
                    <th style="padding: 8px 12px; text-align: center; font-size: 12px; font-weight: 600; color: #6b7280; border-bottom: 2px solid #e5e7eb;">Días</th>
                  </tr>
                </thead>
                <tbody>
                  ${itemRows}
  ...
  const mailOptions = {
    from: getFormattedSender(),
    to: adminEmail,
    subject: `Alerta: ${items.length} producto(s) recibido(s) pendiente(s) de confirmación`,
    html,
    ...(logoAttachment ? { attachments: [logoAttachment] } : {}),
  };
...
// Send alert email to admin about items stuck in "sent" status
const sendStaleSentAlertEmail = async (items) => {
  const config = require('../config/env');
  const adminEmail = config.registrationEmail;
  if (!adminEmail) {
    logger.warn('No REGISTRATION_EMAIL configured, skipping stale sent alert email');
    return;
  }
...
<head>
  ...
  <meta name="color-scheme" content="light only">
  <meta name="supported-color-schemes" content="light only">
  <title>Alerta: Productos enviados sin recibir</title>
  <style>
    :root { color-scheme: light only; }
    @media (prefers-color-scheme: dark) {
      body, table, td, div { background-color: #ffffff !important; color: #111827 !important; }
...
<body bgcolor="#ffffff" style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #ffffff;">
    ...
    <tr>
      <td align="center" bgcolor="#ffffff" style="background-color: #ffffff;">
        <table width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 40px;">
              <h1 style="margin: 0 0 20px; font-size: 24px; font-weight: 600; color: #111827;">Alerta: Productos enviados sin recibir</h1>
              <p style="margin: 0 0 20px; font-size: 16px; line-height: 1.5; color: #374151;">
                Los siguientes <strong>${items.length}</strong> producto(s) llevan más de 15 días en estado <strong>"enviado"</strong> sin ser marcados como recibidos:
              </p>
              <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse; margin: 0 0 20px;">
                <thead>
                  <tr style="background-color: #f9fafb;">
  ...
  const mailOptions = {
    from: getFormattedSender(),
    to: adminEmail,
    subject: `Alerta: ${items.length} producto(s) enviado(s) sin confirmar recepción`,
    html,
    ...(logoAttachment ? { attachments: [logoAttachment] } : {}),
  };
...
const sendEventConfirmationEmail = async ({ email, firstName, eventTitle, accessPassword, amountPaid }) => {
  const logoAttachment = getLogoAttachment();

  const paymentLine = amountPaid
    ? `<p style="color: #374151; line-height: 1.6; margin: 0 0 24px;">Pago realizado: <strong>${amountPaid}\u00a0€</strong></p>`
    : '';
...
<body bgcolor="#ffffff" style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #ffffff; background: #ffffff;">
    ...
    <tr>
      <td align="center" bgcolor="#ffffff" style="background-color: #ffffff;">
        <table width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 20px 40px 40px;">
              <h1 style="margin: 0 0 20px; font-size: 24px; font-weight: 600; color: #111827;">Registro confirmado</h1>
              <p style="color: #374151; line-height: 1.6; margin: 0 0 16px;">Hola <strong>${firstName}</strong>,</p>
              <p style="color: #374151; line-height: 1.6; margin: 0 0 24px;">Tu registro en el evento <strong>${eventTitle}</strong> ha sido confirmado.</p>
              ${paymentLine}
              <div style="background-color: #fef3c7; border: 1px solid #f59e0b; border-radius: 8px; padding: 20px; margin: 0 0 24px;">
                <p style="color: #92400e; margin: 0 0 8px 0; font-weight: bold;">Tu contraseña de acceso:</p>
                <p style="color: #92400e; margin: 0; font-size: 28px; font-weight: bold; letter-spacing: 4px; text-align: center;">${accessPassword}</p>
                <p style="color: #92400e; margin: 8px 0 0 0; font-size: 12px;">Guarda esta contraseña. La necesitarás para acceder al evento desde otro dispositivo o si se borran los datos del navegador.</p>
              </div>
              <p style="color: #6b7280; font-size: 12px; margin-top: 32px; text-align: center;">\u00a9 ${new Date().getFullYear()} 140d Galería de Arte. Todos los derechos reservados.</p>
            </td>
          </tr>
  ...
  try {
    ...
    logger.error({ err: error }, 'Error sending event confirmation email');
    return { success: false };
  }
};

const sendEventVerificationEmail = async ({ email, code }) => {
  const logoAttachment = getLogoAttachment();
...
<body bgcolor="#ffffff" style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #ffffff; background: #ffffff;">
  ...
  try {
    ...
    logger.error({ err: error }, 'Error sending event verification email');
    return { success: false };
  }
};
...
// Alert the business inbox when a scheduled database backup fails (change:
// turso-s3-backups). A silent failure is the worst possible outcome for a
// backup system — you only find out the day you need to restore — so the
// failure path is loud on three channels: log, Sentry and this email. Under
// NODE_ENV=test the `noop` transport keeps it in the in-memory outbox.
const sendBackupFailureEmail = async ({ env, dateKey, key, error }) => {
  const recipient = config.business.email;
  if (!recipient) {
    logger.warn('No BUSINESS_EMAIL/EMAIL_FROM configured, skipping backup failure alert email');
...
<head>
  ...
  <meta name="color-scheme" content="light only">
  <meta name="supported-color-schemes" content="light only">
  <title>Fallo en la copia de seguridad de la base de datos</title>
  <style>
    :root { color-scheme: light only; }
    @media (prefers-color-scheme: dark) {
      body, table, td, div { background-color: #ffffff !important; color: #111827 !important; }
...
<body bgcolor="#ffffff" style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #ffffff;">
    ...
    <tr>
      <td align="center" bgcolor="#ffffff" style="background-color: #ffffff;">
        <table width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 20px 40px 40px;">
              <h1 style="margin: 0 0 20px; font-size: 22px; font-weight: 600; color: #111827;">Fallo en la copia de seguridad de la base de datos</h1>
              <p style="margin: 0 0 20px; font-size: 16px; line-height: 1.5; color: #374151;">
                La copia de seguridad automática <strong>no se ha completado</strong>. La base de datos no ha sufrido ningún cambio (el proceso es de solo lectura), pero <strong>no existe copia de este día</strong> hasta que se resuelva.
              </p>
              <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse; margin: 0 0 24px;">
                <tbody>
                  ${row('Entorno', env || '—')}
                  ${row('Fecha de la copia', dateKey || '—')}
                  ${row('Destino', key || '—')}
                  ${row('Momento del fallo', failedAt)}
                  ${row('Error', message)}
                </tbody>
              </table>
              <p style="margin: 0; font-size: 14px; line-height: 1.5; color: #374151;">
                Revisa los logs del contenedor de la api y Sentry para el detalle completo. Se puede relanzar a mano con <code style="background-color: #f3f4f6; padding: 2px 6px; border-radius: 4px;">docker compose exec api npm run backup:now</code>.
              </p>
            </td>
          </tr>
          <tr>
  ...
  const mailOptions = {
    from: getFormattedSender(),
    to: recipient,
    subject: `[${env || 'api'}] Fallo en la copia de seguridad de la base de datos (${dateKey || 'sin fecha'})`,
    html,
    ...(logoAttachment ? { attachments: [logoAttachment] } : {}),
  };

  const info = await sendMail(mailOptions);
  logger.info({ recipient, messageId: info.messageId, env, dateKey }, 'Backup failure alert email sent');
  return { success: true, messageId: info.messageId };
};
...
// Quote request email. Independent from sendArtInquiryEmail above (it carries an
// extra "Código postal de envío" row and "Solicitud de cotización" wording).
const sendQuoteRequestEmail = async ({ inquiry, product }) => {
  const recipient = config.business.email;
  const clientUrl = config.clientUrl || 'http://localhost:3000';
  const productUrl = `${clientUrl}/galeria/p/${product.id}`;
  const priceFmt = typeof product.price === 'number'
...
<body bgcolor="#ffffff" style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #ffffff;">
    ...
    <tr>
      <td align="center" bgcolor="#ffffff" style="background-color: #ffffff;">
        <table width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 20px 40px 40px;">
              <h1 style="margin: 0 0 20px; font-size: 22px; font-weight: 600; color: #111827;">Nueva solicitud de cotización</h1>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse;">
                ...
                ${phoneLine}
                <tr>
                  <td style="padding: 8px 12px; font-size: 14px; font-weight: 600; color: #6b7280; border-bottom: 1px solid #e5e7eb;">Código postal de envío</td>
                  <td style="padding: 8px 12px; font-size: 14px; color: #374151; border-bottom: 1px solid #e5e7eb;">${escapeForEmail(inquiry.postalCode)}</td>
                </tr>
              </table>

              <h2 style="margin: 24px 0 8px; font-size: 16px; font-weight: 600; color: #111827;">Más información</h2>
              <div style="padding: 12px 16px; background-color: #f9fafb; border-left: 3px solid #111827; font-size: 14px; line-height: 1.6; color: #374151;">${escapedMessage}</div>
              ...
              <p style="margin: 32px 0 0; font-size: 13px; color: #6b7280;">
                Para responder a esta solicitud, basta con pulsar "Responder" en tu cliente de correo: el destinatario ya es el email del usuario.
              </p>
            </td>
          </tr>
          <tr>
  ...
  const mailOptions = {
    ...
    to: recipient,
    replyTo: inquiry.email,
    subject: `[Cotización] ${product.name} (#${product.id})`,
    html,
    ...(logoAttachment ? { attachments: [logoAttachment] } : {}),
  };
