...
* **Known blind spot:** the console's layout, the mic meter and the fullscreen behaviour have **no automated test** — `client/` still has no test runner. The API half (column, validators, persistence) is covered by `api/tests/mobileHostConsoleFlag.test.js`, which also asserts the four-place write path by name.
