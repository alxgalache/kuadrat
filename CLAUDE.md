...
* **Known gap, not fixed here:** Sendcloud shipping for `other` products appears not to be charged — `computeShippingTotal` sums `item.shipping.cost`, which is `null` for those items, while the cart UI and `placeOrder` both include it. Unverified against a real order; see `openspec/changes/shipping-cost-verification/design.md` § "Hallazgo fuera de alcance".
