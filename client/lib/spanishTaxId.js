 ...
 *
 * Legal-entity CIFs (`B12345678`) are DELIBERATELY rejected: this answers "who
 * is the buyer" in the cart checkout, where the decision was to accept
 * individuals only. `SellerFiscalForm.js` answers a different question — the
 * SELLER's fiscal identity, where a company is legitimate — and keeps its own,
 * format-only, CIF-accepting rules. Do not merge the two.
 *
 * MIRRORED IN `api/utils/spanishTaxId.js`. The monorepo has no shared package
 * and the two apps build into separate images, so the algorithm exists twice on
 * purpose; any change here must be applied there.
 */
 ...
 * @returns {string}
 */
export function normalizeSpanishTaxId(value) {
 ...
 * @returns {boolean}
 */
export function validateSpanishTaxId(value) {
