...
import { artAPI, authorsAPI, getArtImageUrl } from '@/lib/api'
...
export default function GalleryPage() {
  ...
  const loadAuthors = async () => {
    try {
      const authorsData = await authorsAPI.getVisible('art')
  ...
  const loadProducts = async (resetPage = false) => {
    try {
      if (resetPage) {
        ...
        setPage(1)
        const productsData = await artAPI.getAll(1, 12, selectedAuthorSlug)
        ...
        const productsData = await artAPI.getAll(nextPage, 12, selectedAuthorSlug)
  ...
  const handleFilterByAuthor = (authorSlug) => {
    ...
    if (selectedAuthorSlug === authorSlug) {
      // Deselect - go to /galeria with no params
      router.push('/galeria')
    } else {
      // Select - add author to URL
      router.push(`/galeria?author=${authorSlug}`)
    }
  }
                        ...
                        <div className="group relative">
                          ...
                          <div className="mt-6">
                            ...
                            <h3 className="mt-1 font-semibold text-gray-900">
                              <Link href={`/galeria/p/${product.slug}`}>
