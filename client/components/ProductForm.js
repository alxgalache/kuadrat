...
// Shared product form used by the seller publish page (mode="create") and the
// admin product edit page (mode="edit").
//
// Image slots are either null (empty), { file, previewUrl } (new upload), or
// { basename, previewUrl } (existing image, edit mode only — distinguished by
// the absence of `file`). In edit mode the submit FormData carries an
// `images_manifest` (and `variation_<idx>_images_manifest`) describing kept
// existing images plus `new` placeholders matching the appended files, which
// the backend reconciles.
//
// Props:
// - mode: 'create' | 'edit'
// - initialProduct (edit): product from the admin edit-data endpoint, with
...
export default function ProductForm({
    ...
    // Existing images preview through the public image endpoints; variation
    // images always live under the others storage path.
    const toExistingSlot = (img, category) => ({
        basename: img.basename,
        previewUrl: category === 'art' ? getArtImageUrl(img.basename) : getOthersImageUrl(img.basename),
    })

    const initialNamedVariations = isEdit
        ? (initialProduct?.variations || []).filter((v) => v.key != null && String(v.key).trim() !== '')
        : []
    const initialGlobalVariation = isEdit
        ? (initialProduct?.variations || []).find((v) => v.key == null || String(v.key).trim() === '')
        : null
    // Keeping the anonymous variation's id lets the backend update its stock
    // in place instead of delete + reinsert.
    const globalVariationId = initialGlobalVariation?.id ?? null

    const [productCategory, setProductCategory] = useState(isEdit ? initialProductType : 'art')
    ...
    const [weight, setWeight] = useState(initialProduct?.weight != null ? String(initialProduct.weight) : '')
    ...
    const [imageSlots, setImageSlots] = useState(() => {
        if (isEdit && initialProduct?.images?.length) {
            return initialProduct.images.map((img) => toExistingSlot(img, initialProductType))
        }
        return [null]
    })
    const [loading, setLoading] = useState(false)
    const [showDecimalWarning, setShowDecimalWarning] = useState(false)
    // Per-seller gallery commission rates (whole percentages). In create mode
    // they are fetched from the seller API; in edit mode the caller supplies
    // the product owner's rates.
    ...
    const [hasVariations, setHasVariations] = useState(initialNamedVariations.length > 0)
    const [globalStock, setGlobalStock] = useState(
        initialGlobalVariation?.stock != null ? String(initialGlobalVariation.stock) : ''
    )
    const [variations, setVariations] = useState(() =>
        initialNamedVariations.length > 0
            ? initialNamedVariations.map((v) => ({
                id: v.id,
                key: v.key,
                stock: v.stock != null ? String(v.stock) : '',
                imageSlots: v.images?.length ? v.images.map((img) => toExistingSlot(img, 'other')) : [null],
              }))
    ...
    // Build a manifest entry per filled slot: existing images by basename,
    // new uploads as ordered placeholders (edit mode only).
    const slotsToManifest = (slots) =>
        slots.map((slot) => (slot.file ? { kind: 'new' } : { kind: 'existing', basename: slot.basename }))
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Append each new global image under the `images` multipart field.
            // In edit mode, also describe the final slot order (kept existing
            // images + new placeholders) for the backend to reconcile.
            if (isEdit) {
                formData.append('images_manifest', JSON.stringify(slotsToManifest(filledGlobalSlots)))
            }
            ...
            if (productCategory === 'art') {
                ...
                const variationsData = hasVariations
                    ? variations.map((v) => {
                        ...
                        if (isEdit && v.id != null) entry.id = v.id
                        return entry
                      })
                    : [
                        isEdit && globalVariationId != null
                            ? { id: globalVariationId, key: null, stock: parseInt(globalStock, 10) }
                ...
                if (hasVariations) {
                    variations.forEach((v, varIdx) => {
                        ...
                        if (isEdit) {
                            formData.append(`variation_${varIdx}_images_manifest`, JSON.stringify(slotsToManifest(files)))
                        }
                        for (const slot of files) {
                            if (slot.file) formData.append(`variation_${varIdx}_images`, slot.file)
                        }
                    })
                }
            }

            await onSubmit(formData, productCategory)
        } catch (err) {
            showApiError(err)
        } finally {
            setLoading(false)
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-2 grid grid-cols-1">
                                            <select
                                                ...
                                                disabled={isEdit}
                                                className="col-start-1 row-start-1 w-full appearance-none rounded-md border border-gray-300 bg-white py-1.5 pr-8 pl-3 text-base text-gray-900 focus:border-black focus:ring-2 focus:ring-black sm:text-sm/6 disabled:bg-gray-100 disabled:text-gray-500"
                                            >
