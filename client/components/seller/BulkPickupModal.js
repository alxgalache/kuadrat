...
export default function BulkPickupModal({ isOpen, onClose, carriers, orders, defaultAddress, onSuccess }) {
  ...
  // Orders filtered by selected carrier
  const filteredOrders = orders.filter(o =>
    o.items?.some(i => i.sendcloudCarrierCode === selectedCarrier)
  )
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      await sellerAPI.scheduleBulkPickup({
      ...
      setError(err.message || 'Error al programar la recogida masiva')
    } finally {
      setLoading(false)
    }
  }, [address, timeSlotStart, timeSlotEnd, specialInstructions, selectedOrderIds, validate, onSuccess, onClose])
        ...
        <div className="relative w-full max-w-lg transform rounded-lg bg-white p-6 shadow-xl transition-all">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">
              ...
              {step > 1 && (
                <span className="ml-2 text-sm font-normal text-gray-500">Paso {step} de 3</span>
              )}
            </h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => { setSelectedCarrier(e.target.value); setSelectedOrderIds([]) }}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm text-gray-700">
                  {filteredOrders.length} pedido{filteredOrders.length !== 1 ? 's' : ''} con {formatCarrierName(selectedCarrier)}
                </p>
                ...
                {filteredOrders.map(order => (
                  <label
                    ...
                    className={`flex cursor-pointer items-start gap-3 rounded-md border p-3 transition-colors ${
                      selectedOrderIds.includes(order.orderId) ? 'border-black bg-gray-50' : 'border-gray-200 hover:bg-gray-50'
                    }`}
                  >
                    <input
                      type="checkbox"
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-900">Pedido #{order.orderId}</p>
                      {order.deliveryAddress && (
                        <p className="text-xs text-gray-500 truncate">{formatDeliveryAddress(order.deliveryAddress)}</p>
                      )}
                    </div>
                  </label>
                ))}
          ...
          {/* Step 3: Address + time slot form */}
          {step === 3 && (
            <form onSubmit={handleSubmit} className="space-y-4">
              <p className="text-sm text-gray-500">
                {selectedOrderIds.length} pedido{selectedOrderIds.length !== 1 ? 's' : ''} seleccionado{selectedOrderIds.length !== 1 ? 's' : ''} — {formatCarrierName(selectedCarrier)}
              </p>
