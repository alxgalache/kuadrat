...
#### Scenario: Shipments created after payment confirmation
- **WHEN** `processOrderConfirmation()` completes successfully and the order contains items with Sendcloud shipping
- **THEN** the system SHALL call `SendcloudProvider.createShipments()` for each seller/parcel group, creating one shipment per parcel via `POST /v3/shipments` (asynchronous endpoint)

#### Scenario: Shipment data stored on order items
- **WHEN** Sendcloud shipments are created successfully
- **THEN** the `sendcloud_shipment_id` (shipment UUID), `sendcloud_parcel_id` (parcel integer ID) SHALL be stored on the corresponding `art_order_items` or `other_order_items` records. The `tracking` and `sendcloud_tracking_url` fields SHALL remain null initially (populated later via webhook).

#### Scenario: Shipment creation failure does not block order
- **WHEN** Sendcloud shipment creation fails (API error, timeout)
- **THEN** the order SHALL remain in `paid` status, the error SHALL be logged with Pino, and the order item SHALL be left with `sendcloud_shipment_id` set but `sendcloud_parcel_id` null, making it eligible for the retry scheduler.
...
#### Scenario: Webhook receives status update
- **WHEN** Sendcloud sends a webhook notification with a parcel status change
- **THEN** the system SHALL look up the order item by `sendcloud_parcel_id` first (falling back to `sendcloud_shipment_id` via `parcel.shipment_uuid`), map the Sendcloud status to the internal status, and update the order item
...
#### Scenario: Status mapping — ready to send
- **WHEN** the webhook reports Sendcloud status 1000 (ready to send)
- **THEN** the system SHALL update tracking info from the payload, store the `sendcloud_parcel_id` if not already stored, and trigger the seller label notification email

#### Scenario: Webhook signature validation
- **WHEN** `SENDCLOUD_WEBHOOK_SECRET` is configured
- **THEN** the webhook endpoint SHALL validate the request signature using HMAC-SHA256 against the raw request body and reject requests with invalid signatures with a 401 response

#### Scenario: Unknown shipment/parcel ID
- **WHEN** the webhook receives a notification for a parcel ID and shipment UUID not found in the database
- **THEN** the system SHALL log a warning and return 200 (acknowledge receipt to prevent retries)
...
The `art_order_items` and `other_order_items` tables SHALL include columns for Sendcloud shipment and parcel tracking data.

#### Scenario: Columns on order items tables
- **WHEN** the database is initialized
- **THEN** both `art_order_items` and `other_order_items` tables SHALL include `sendcloud_shipment_id TEXT`, `sendcloud_parcel_id TEXT`, `sendcloud_tracking_url TEXT`, `sendcloud_announcement_retries INTEGER DEFAULT 0`, and `sendcloud_announcement_failed_at DATETIME` columns

#### Scenario: Tracking data populated via webhook
- **WHEN** a Sendcloud webhook delivers tracking information for an order item
- **THEN** `tracking` SHALL contain the carrier tracking number and `sendcloud_tracking_url` SHALL contain the public tracking URL
...
- **WHEN** a Sendcloud shipment is created for a seller's products
- **THEN** the system SHALL send an email to the seller with order details and item list. The label will arrive in a separate email when it becomes available.
