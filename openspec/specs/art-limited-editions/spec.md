...
An `art` row may represent a run of N copies (e.g. 15 prints of a digital collage) instead of a single unique physical work. Two columns on `art` model it — `edition_size`, fixed at creation and immutable, and `editions_sold`, the number of copies reserved or sold — and `is_sold` is redefined to mean "edition sold out" so that every existing reader of that flag keeps working untouched. With `edition_size = 1` the behavior is bit-for-bit the pre-existing one.
