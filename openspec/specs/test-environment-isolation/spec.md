...
## Purpose
Garantizar que la suite de tests del backend se ejecute completamente aislada del mundo exterior: base de datos local efímera en lugar de la Turso de preproducción, transporte de email inerte en lugar del proveedor real, e importación de la aplicación sin efectos secundarios (puertos, esquema, schedulers). El aislamiento SHALL estar impuesto por código (guardias que abortan el proceso), no por convención.
