...
Dos tipos de vendedor bajo el mismo `role = 'seller'`: **Artista**, el vendedor completo que existe hoy —publica obra, publica tienda, gestiona envíos y puede ser host de eventos—, y **Ponente**, el invitado que sólo participa en eventos multimedia (streams en directo, charlas, pases de vídeo, talleres y cursos), cobra su parte y no publica nada. La distinción vive en `users.seller_kind` (`DEFAULT 'artist'`, desplegada sin backfill, de modo que toda cuenta existente conserva exactamente su comportamiento previo), se traduce en un único conjunto de capacidades por tipo definido en un solo sitio por lado, y sólo la escribe el admin.

La prohibición se aplica en el servidor con `requireArtistSeller`: ocultar una entrada de menú no es un permiso. Cambiar el tipo corta las sesiones abiertas de ese vendedor (`users.sessions_invalidated_at`, escrito en el mismo `UPDATE`), se rechaza con 409 `SELLER_KIND_CHANGE_BLOCKED` mientras haya huella comercial viva o un directo en curso, y nunca despublica, oculta ni borra nada de forma automática.

> Capa afectada: backend (`api/`) y frontend (`client/`). Cambia el esquema de BD: SOLO vía `api/config/database.js` (`CREATE TABLE` + `safeAlter`).
