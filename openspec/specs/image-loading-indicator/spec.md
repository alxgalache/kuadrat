...
Cuándo aparece —y sobre todo cuándo NO aparece— un indicador de carga sobre el hueco gris que reserva una imagen de producto o de evento antes de llegar. Sin él, ese `bg-gray-200` es indistinguible del que deja una imagen rota o un producto sin foto, que es la lectura que menos conviene en una galería donde la imagen *es* el producto.

Casi todo lo que aquí se declara existe por una trampa concreta, y tres son invisibles hasta que muerden. **`onLoad` no basta**: es un manejador de React y si la imagen ya estaba completa cuando React lo adjuntó —caché del navegador, vuelta atrás del historial— el evento ya se disparó y no vuelve, dejando el indicador encendido para siempre encima de una imagen visible. **El retardo no es sólo estética**: al vivir en un efecto, que no corre en el servidor, es también lo único que impide un desajuste de hidratación en las rutas renderizadas en servidor. Y **un indicador sobre una imagen `loading="lazy"` que aún no se ha pedido no dice «esto viene en camino», miente**, y además gira indefinidamente sobre celdas que nadie mira.

Las reglas de coste no hablan de «propiedades compuestas» sino del área repintada, que es lo que de verdad protegen: la animación elegida transforma el SVG y anima el trazo del círculo, y eso repinta el anillo, nunca la caja.

> Capa afectada: `client/hooks/useImageLoaded.js`, `client/components/ImageLoadingPlaceholder.js`, el bloque `.image-loader` de `client/app/globals.css`, y las nueve superficies públicas que pintan una imagen de producto o de evento.
