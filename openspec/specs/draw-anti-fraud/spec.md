...
The system SHALL capture and store the client IP address when a draw buyer is registered. The IP SHALL be stored in `draw_buyers.ip_address` for admin review purposes only — no automated blocking based on IP.

#### Scenario: IP captured at registration
- **WHEN** `POST /api/draws/:id/register-buyer` is called
- **THEN** the system SHALL store `req.ip` in the `draw_buyers.ip_address` column
...
- **WHEN** multiple `draw_buyers` records exist with the same IP address for the same draw
