...
Permitir a un visitante de la ficha pública de una obra (`/galeria/p/[id]`) enviar una consulta directa al buzón comercial (otro método de pago, otro método de envío, información específica) sin abandonar la página. Las consultas se envían por email a `BUSINESS_EMAIL`; no se persisten en la base de datos. El endpoint público se protege con Cloudflare Turnstile y un rate limit dedicado.
...
- **WHEN** `config.business.email` está definida y la consulta supera todas las validaciones
- **THEN** se envía un email a `config.business.email` con `Reply-To: <user_email>` y todos los campos del formulario y de la obra en el cuerpo.

#### Scenario: Fallback a EMAIL_FROM

- **WHEN** `config.business.email` NO está definida y la consulta supera todas las validaciones
- **THEN** se envía un email a `config.emailFrom` con el mismo formato y `Reply-To: <user_email>` (el fallback ya está implementado en `config.business.email`).
...
El cliente SHALL ofrecer feedback claro tras el envío usando `BannerNotificationContext`. En éxito, cierra el modal y muestra banner verde "Consulta enviada. Te responderemos en breve". En error recuperable (captcha fallido, rate limit, fallo de email), deja el modal abierto, resetea el widget de Turnstile, y muestra banner rojo con mensaje específico. En error no recuperable desde dentro del modal (captcha no disponible), cierra el modal, limpia el estado del formulario, y muestra banner rojo.
...
- **WHEN** la API responde 503 con código `CAPTCHA_UNAVAILABLE`
- **THEN** se muestra un banner "No se puede enviar la consulta en este momento. Inténtalo más tarde"
- **AND** el modal se cierra
- **AND** los valores del formulario se vacían (el usuario no puede recuperarse desde dentro del modal porque la verificación es indispensable).
...
- `BUSINESS_EMAIL` (opcional; reutilizada vía `config.business.email`, que ya hace fallback a `EMAIL_FROM` cuando no está definida).
- `TURNSTILE_SECRET` (opcional; si está vacía, el endpoint responde 503 con `CAPTCHA_UNAVAILABLE`).
- `NEXT_PUBLIC_TURNSTILE_SITE_KEY` (cliente; si está vacía, el enlace "haz click aquí" SHALL ocultarse).
- `INQUIRY_RATE_LIMIT_MAX_REQUESTS` (opcional, default 3).
- `INQUIRY_RATE_LIMIT_WINDOW_SECONDS` (opcional, default 60 = 1 hora; recordando que el limiter multiplica por 60).

Estas variables SHALL documentarse en `api/.env.example` (y `client/.env.example` para la variable pública), en el `.env.example` raíz, en los `Dockerfile.prod`/`Dockerfile.staging` del cliente, en los `docker-compose.{prod,pre2}.yml`, y en la sección "Environment Variables" de `CLAUDE.md`.

#### Scenario: BUSINESS_EMAIL ausente

- **WHEN** el proceso arranca sin `BUSINESS_EMAIL` definida y con `EMAIL_FROM` definida
- **THEN** `config.business.email === config.emailFrom`
- **AND** los emails de consulta se envían a `config.emailFrom`.

#### Scenario: TURNSTILE_SECRET ausente en producción

- **WHEN** `config.turnstile.secret` no está definida y el endpoint recibe una petición
