...
Give buyers a way to view product images full and uncropped from the art and store detail pages. The detail carousel exposes a "Ver imagen completa" affordance on every image and opens a modal lightbox that displays the whole image (letterboxed against a darkened backdrop), supports mouse-wheel magnifier zoom, and lets the user navigate across all of the product's images. The product grid keeps its existing cropped presentation.
