...
## Purpose
Reglas del limitador general de la API en producción: qué tráfico consume cuota y qué tráfico queda exento. El renderizado del servidor de Next.js llama a la API desde dentro del despliegue y no lleva la IP del visitante, de modo que sin una exención explícita todos los renders comparten una única cubeta y el sitio se autolimita bajo carga.
