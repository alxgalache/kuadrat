# sentry-environment-gating Specification

## Purpose
Gating explícito por entorno del envío de eventos a Sentry, de modo que el stream de issues sólo contenga señal de staging y producción. Desarrollo mantiene el SDK cableado pero con el transporte apagado (los errores del ciclo editar-guardar-recargar no son defectos), test excluye Sentry por completo por razones estructurales, y un escape hatch permite reactivar el envío en local para depurar la propia integración.
