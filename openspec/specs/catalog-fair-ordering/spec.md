# catalog-fair-ordering

## Purpose

Repartir de forma equitativa la primera fila de las rejillas públicas de catálogo (`/galeria`, `/tienda`) entre los artistas, en lugar de fijarla por fecha de alta. `GET /api/art` y `GET /api/others` ordenan hoy por `created_at DESC, id DESC`; como cada artista sube su obra de una sentada, ese orden agrupa el catálogo en bloques contiguos por artista y condena permanentemente a quien subió primero a no aparecer nunca en la primera página. Esta capacidad añade un parámetro `seed` opcional que, cuando llega, ordena el catálogo entrelazando artistas por rondas de forma determinista y aleatorizada; sin semilla, o con filtro por autor, el comportamiento cronológico de siempre queda intacto.
