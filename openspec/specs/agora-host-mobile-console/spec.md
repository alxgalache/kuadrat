...
Tres modos de vista intercambiables para el host de una sala Agora en modo `broadcast` —`full` (la vista completa de siempre), `console` (un panel de operación pensado para un móvil en horizontal) y `preview` (solo el vídeo)—, habilitados por evento con `events.allow_mobile_host_console`. Nacen de retransmitir conferencias desde un teléfono montado en un trípode: en horizontal Chrome deja ~300 px CSS de alto, y la vista de escritorio deja los controles bajo el pliegue. Cubre además el permiso por evento para que el host elija la resolución de emisión (`events.allow_host_video_quality`), palanca de coste en manos del admin.

> Capa afectada: backend (`api/`) y frontend (`client/`). Cambia el esquema de BD: SOLO vía `api/config/database.js` (`CREATE TABLE` + `safeAlter`). Los asistentes, los eventos LiveKit y el modo `meeting` conservan su comportamiento; un evento sin los flags marcados se comporta exactamente igual que antes del cambio.
