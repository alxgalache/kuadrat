# catalog-listing-seeding

## Purpose

Los listados `/galeria` y `/tienda` se sirven construidos desde el servidor (ISR) y el cliente no repite al montar la petición cuyo resultado ya está en el HTML: productos, semilla de ordenación, `hasMore` y la lista de autores del filtro viajan sembrados. La restauración de scroll y la siembra incompleta siguen pidiendo lo que necesitan.

> Capa afectada: `client/lib/serverApi.js`, `client/hooks/useGalleryProducts.js`, `client/hooks/useGalleryAuthors.js`, las páginas de `/galeria`, `/tienda` y sus fichas de artista.
