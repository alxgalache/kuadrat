...
An admin acting as another user through a short-lived, audited, password-free token exchange. Artists regularly ask the gallery to do something inside their own account — publish a piece, fix a shipping configuration, read a screen that only exists for `role = 'seller'` — and until now the only way to satisfy that was for the artist to hand over their password, which puts a live credential in a chat message, defeats `password_changed_at` and the admin-initiated reset flow, and leaves no way to tell afterwards whether an action was the artist's or the gallery's.

`POST /api/admin/impersonation/:userId/start` mints a 60-minute JWT whose subject is the target user and which carries an `act` claim naming the admin; `POST /api/auth/impersonation/stop` reads that claim, re-validates the admin against the database and mints a fresh admin token. Because `api/config/passport.js` rebuilds `req.user` from `jwtPayload.id` on every request, the resulting session is indistinguishable from a real login for every existing screen, guard and endpoint — so no existing `req.user.id` or `req.user.role` check needs modification.

Guardrails are deliberately few: the target must not be `role = 'admin'` (no lateral or upward moves) and `PUT /api/seller/profile/password` is refused while impersonating. Everything else the artist can do, the admin can do. `impersonation_sessions` records who impersonated whom, when it started, when and how it ended, and from which IP.

> Layer affected: `api/controllers/impersonationController.js`, `api/routes/admin/impersonationRoutes.js`, `api/routes/authRoutes.js`, `api/config/passport.js`, `api/middleware/authorization.js`, `api/routes/sellerRoutes.js`, `api/app.js`, `client/contexts/AuthContext.js`, `client/lib/api.js`, `client/components/Navbar.js`, `client/app/admin/autores/page.js`, plus the new `impersonation_sessions` table.
