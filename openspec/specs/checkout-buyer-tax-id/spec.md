...
La captura, validación y persistencia del identificador fiscal (DNI/NIE) del comprador en el checkout del carrito. El carrito recogía nombre, email y teléfono, pero no el NIF, de modo que la compra directa era el único canal de venta que llegaba a facturación sin identificar al comprador — sorteos y subastas ya lo exigen en `draw_buyers.dni` y `auction_buyers.dni`.

El campo se valida con el **dígito de control** del NIF español (no solo el formato) desde un único módulo por lado — `api/utils/spanishTaxId.js` y `client/lib/spanishTaxId.js` — es obligatorio para avanzar en el checkout, y se persiste en la columna nullable `orders.dni`, que los pedidos de sorteo y subasta también rellenan propagando el DNI que ya tienen. La columna es nullable a propósito: los pedidos anteriores al cambio siguen siendo válidos y facturables.
