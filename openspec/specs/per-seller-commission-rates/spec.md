...
Gallery commission is stored per seller (not as global env vars): each seller
carries their own commission percentage for `art` and `other` products on the
`users` table. Every sale calculation (cart checkout, auction billing, draw
billing, paid-event host credit) derives the commission from the rate configured
on the seller that owns the product, and admins manage those rates per author.
