...
Devolver al usuario a la posición exacta del grid cuando vuelve atrás desde el detalle de un producto. Los grids de galería y tienda tienen scroll infinito y se remontan desde cero al volver, de modo que la restauración nativa del navegador no encuentra contenido al que volver y el usuario aparece arriba del todo con solo la primera página cargada. La instantánea se asocia a la entrada del historial, no a la URL, para que solo la navegación atrás/adelante active la restauración.

Aplica a los cuatro grids que comparten `client/components/ProductGrid.js`: `/galeria`, `/tienda`, `/galeria/autor/[authorSlug]` y `/tienda/autor/[authorSlug]`.
