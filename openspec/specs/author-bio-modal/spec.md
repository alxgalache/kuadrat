...
Presentar la ficha del artista (`AuthorModal`) como una ficha de catálogo legible, y renderizar fielmente la biografía enriquecida que el admin escribe con el editor Quill.

En escritorio la ficha se organiza en dos columnas —imagen del artista a la izquierda ocupando toda la altura del panel, contenido textual a la derecha—, con la imagen persistente mientras se recorre la biografía y el panel acotado para que nunca sobrepase el alto de la ventana. Por debajo de `md` la maqueta se apila con la imagen apaisada arriba y el botón "Cerrar" fijo en la barra inferior.

Como la banda apilada de móvil y la columna de escritorio tienen proporciones opuestas y extremas, una sola fotografía no puede llenar ambas conservando su sujeto: cada artista dispone por tanto de dos imágenes (`profile_img` y `profile_img_mobile`) y de un indicador `hide_profile_img_mobile` para suprimir la imagen en pantallas pequeñas. Un artista con una sola imagen se comporta exactamente igual que antes.

El renderizado de la biografía no depende de `@tailwindcss/typography` (no instalado): unos estilos propios acotados al contenedor restituyen párrafos, marcadores y sangrías, derivando el tipo de lista del atributo `data-list` que Quill 2 coloca en cada `li` —las viñetas se codifican como `<ol><li data-list="bullet">`, de modo que estilar por etiqueta las numeraría—. El mismo tratamiento se aplica en el perfil del vendedor y en la ficha de autor del panel de administración.

El componente es compartido: la ficha aparece en la galería de arte, la tienda, las páginas de autor, el detalle de producto, los sorteos y los eventos en directo, con el mismo contrato de props (`author`, `open`, `onClose`).
