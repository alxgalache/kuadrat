...
- [ ] 19.7 Manual smoke: create one art with 1 image, one art with 3 images, one others with no variation images, one others with 2 variations × 2 images each; verify gallery grid, tienda grid, detail pages, add-to-cart, checkout, order email, admin views all render images correctly
