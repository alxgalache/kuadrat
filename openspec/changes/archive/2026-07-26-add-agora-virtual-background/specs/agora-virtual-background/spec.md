# agora-virtual-background — Delta Spec (add-agora-virtual-background)

> Nueva capability. Solo frontend: `client/hooks/useAgoraVideoEffect.js`, `client/components/events/VideoEffectsMenu.js`, `client/components/AgoraLiveRoom.js`, `client/hooks/useAgoraRoom.js`, `client/lib/virtualBackgrounds.js`, `client/lib/constants.js` y `client/public/fondos-virtuales/`. Sin cambios en `api/`, en `api/config/database.js`, en variables de entorno ni en el CSP de `client/next.config.js`. LiveKit (`EventLiveRoom.js`) queda intacto.
