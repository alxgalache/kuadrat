...
### Requirement: Missing variation image on submit

**Reason**: Variation images become optional under the new multi-image model. A variation may legitimately have zero images and fall back to displaying the product's global images in the detail page carousel.

**Migration**: Sellers no longer need to upload an image per variation. The form-side scenario "Missing variation image on submit displays a validation error" is removed; submissions with no variation images succeed. Existing backend image validation rules (format, size, dimensions) remain for any uploaded files.
