...
El backend SHALL leer la configuración de marketing (key, segmento newsletter, IDs de topics, remitente y kill-switch) desde variables de entorno validadas, exigiendo la key y los IDs sólo cuando el marketing está habilitado.

#### Scenario: Validación condicional al arranque
- **WHEN** la aplicación arranca con `MARKETING_EMAILS_ENABLED` activado y falta la key o algún ID de segmento/topic requerido
