...
## 6. Bandas verticales (corrección tras verificación visual)

- [x] 6.1 Resortear el orden de artistas en CADA ronda (`ordenDeRonda`) en lugar de fijarlo una vez, conservando la restricción dura «el primero de la ronda no es el último de la anterior».
- [x] 6.2 Exigir además desarreglo respecto de la ronda anterior a partir de `MIN_PARTICIPANTES_COLUMNA` (4), con muestreo por rechazo topado en `INTENTOS_RONDA` y reparación por intercambio como red final.
- [x] 6.3 Corregir el caso de ronda de dos participantes: NO reutilizar `previo`, que al encoger la ronda contiene artistas agotados y rompía la permutación del catálogo.
- [x] 6.4 Tests de ausencia de bandas: cero vecinos verticales con tantas columnas como artistas; ninguna columna monopolizada a 2 y a 4 columnas; la cabeza de ronda rota entre todos los artistas; la secuencia no es periódica.
- [x] 6.5 Suite de escalabilidad: barrido de 1 a 50 artistas y de reparto parejo a un artista que copa el catálogo, comprobando permutación exacta, contigüidad inevitable, cabecera de `k` artistas distintos, equidad de la primera posición y bandeo no superior al del azar puro.
- [x] 6.6 Comprobar que el guardián falla con la implementación anterior (100 % de vecinos verticales iguales y 100 % de columnas monopolizadas).
- [x] 6.7 Verificar en `/galeria` que las columnas mezclan artistas y que no hay repeticiones ni horizontales ni verticales.
