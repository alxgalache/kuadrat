...
- [x] 10.5 **Hecho, en consola:** dar de alta el monitor externo (Sentry Uptime o equivalente). Los dos que valen son `https://api.140d.art/health/ready` y `https://api.140d.art/api/art?page=1&limit=1`: ninguno pasa por la caché, así que un fallo del origen se ve. Vigilar páginas es casi decorativo — `/galeria` se sirve desde caché con `proxy_cache_use_stale` aunque Next esté muerto, y la ficha de obra devuelve 200 aunque la API esté caída, porque `serverApi.js` traga los errores y renderiza «Obra no encontrada». La resiliencia que añadimos es justo lo que hace invisible el fallo desde fuera.
- [x] 10.6 **Hecho, en consola:** alarmas de CloudWatch sobre `StatusCheckFailed` y `CPUUtilization` para `i-08f34f1a05a798185`.
