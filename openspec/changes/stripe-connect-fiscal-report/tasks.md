...
- [ ] 0.1 Verificar que Changes #1, #2 y #3 están merged y desplegados en pre.
- [ ] 0.2 Verificar que en pre existe al menos un payout `completed` de cada régimen (REBU y estándar). Si no, crearlos manualmente siguiendo los flujos de Change #2 y/o #3.
- [ ] 0.3 **Pedir al usuario los datos fiscales del platform** (master plan §9 marca estos campos como pendientes):
  - `BUSINESS_LEGAL_NAME` (razón social legal).
  - `BUSINESS_TAX_ID` (CIF).
  - `BUSINESS_ADDRESS_LINE1`, `LINE2?`, `CITY`, `POSTAL_CODE`, `PROVINCE`.
- [ ] 0.4 Persistir esos valores en el `.env` de pre y producción.

## Fase 1 — Config

- [ ] 1.1 En `api/config/env.js`, añadir un bloque `business`:
  ```js
  business: {
  ...
  ```
- [ ] 1.2 **No bloquear el arranque** si faltan — bloquear sólo en el endpoint del export con 503 (decisión #11 del design).
- [ ] 1.3 Añadir un helper `assertBusinessConfigComplete()` que devuelve la lista de campos faltantes.
- [ ] 1.4 Documentar todas las env vars nuevas en `api/.env.example` con el link al master plan §9.

## Fase 2 — Helpers

- [ ] 2.1 Crear `api/utils/itemDescription.js`:
...
- [ ] 2.2 Crear `api/utils/fiscalReportFormatter.js`:
...
- [ ] 3.1 Crear `api/validators/stripeConnectFiscalReportSchemas.js`:
...
- [ ] 4.1 Crear `api/controllers/stripeConnectFiscalReportController.js`:
...
- [ ] 4.3 Loggear con pino `{ adminId, withdrawalId/range, format }` cada export (auditoría).

## Fase 5 — Routing

- [ ] 5.1 Crear `api/routes/admin/stripeConnectFiscalReportRoutes.js`:
  ```js
...
- [ ] 5.2 Registrar el router en `api/routes/admin/index.js`.
- [ ] 5.3 Verificar que ya tiene `authenticate` + `adminAuth` aplicados desde el índice admin.

## Fase 6 — Frontend — API client

- [ ] 6.1 En `client/lib/api.js`, añadir:
...
- [ ] 6.2 Helper `triggerDownload(blob, filename)` que crea un `<a>` temporal y dispara el click.

## Fase 7 — Frontend — Panel admin

- [ ] 7.1 En `client/app/admin/payouts/[sellerId]/page.js` (ya existe de Change #2):
...
- [ ] 7.2 En `client/app/admin/payouts/page.js` (ya existe de Change #2):
...
- [ ] 7.3 Minimalismo Tailwind por defecto, textos en es-ES.

## Fase 8 — Documentación de la gestoría

- [ ] 8.1 Crear `docs/stripe_connect/fiscal_report_for_gestoria.md` con las 11 secciones listadas en el proposal (sección "Documentación para la gestoría"). Tono: dirigido a la gestoría, terminología fiscal española, ejemplos concretos con números.
- [ ] 8.2 Incluir un ejemplo completo de CSV individual y un fragmento de CSV agregado.
- [ ] 8.3 Incluir un ejemplo del objeto JSON agregado.
- [ ] 8.4 Incluir la tabla de `inferInvoicingMode` con los 4 casos reales (el NULL es error interno).
- [ ] 8.5 Incluir la sección de casos de borde (reversiones, fallos, refunds post-payout).
- [ ] 8.6 Actualizar `docs/stripe_connect/master_plan.md` §13 changelog con un entry para Change #4 y §9 con un enlace al nuevo documento.

## Fase 9 — Testing manual

- [ ] 9.1 En pre, con un payout REBU `completed` existente:
...
- [ ] 9.2 Lo mismo con un payout estándar.
- [ ] 9.3 Lo mismo con un payout `reversed` → verificar que aparece `reversal_amount` y `net_of_reversals`.
- [ ] 9.4 Export agregado del trimestre → verificar que las filas incluyen todos los payouts `completed`/`reversed` y excluyen `failed`.
- [ ] 9.5 Rango de 2 años → 400 con mensaje claro.
- [ ] 9.6 Borrar una env var `BUSINESS_LEGAL_NAME` → 503 con mensaje listando el campo faltante. Restaurar.
- [ ] 9.7 Endpoint `/summary` → verificar totales por régimen.
- [ ] 9.8 Frontend: click en "CSV" descarga archivo con el nombre correcto; click en "Exportar CSV" agregado descarga rango.
- [ ] 9.9 Compartir el CSV individual y el agregado con la gestoría; recoger feedback sobre formato.

## Fase 10 — Go-live

- [ ] 10.1 Completar env vars `BUSINESS_*` en producción.
- [ ] 10.2 Smoke test: exportar un payout real y compartirlo con la gestoría.
- [ ] 10.3 Marcar la iniciativa Stripe Connect como **completa** en `docs/stripe_connect/master_plan.md` §13.
- [ ] 10.4 Actualizar CLAUDE.md / MEMORY.md con una nota "Stripe Connect initiative completed — see master_plan.md" (opcional; el usuario decide).
